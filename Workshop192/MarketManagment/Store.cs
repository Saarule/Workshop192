...
namespace Workshop192.MarketManagment
{
    public class Store
    {
        ...
        public void AddDiscountPolicy(LinkedList<string> policy, int discount)
        {
            ...
            {
                case "AND":
                    discountPolicy = new PolicyCompositeAnd(tmp1, tmp2, 0, name);
                    tmp1.policyId = tmp2.policyId - 1;
                    discountPolicy.policyId = tmp1.policyId - 1;
                    break;
                case "OR":
                    discountPolicy = new PolicyCompositeOr(tmp1, tmp2, 0, name);
                    tmp1.policyId = tmp2.policyId - 1;
                    discountPolicy.policyId = tmp1.policyId - 1;
                    break;
                case "XOR":
                    discountPolicy = new PolicyCompositeXor(tmp1, tmp2, 0, name);
                    tmp1.policyId = tmp2.policyId - 1;
                    discountPolicy.policyId = tmp1.policyId - 1;
                    break;
                default:
        ...
        public void AddSellingPolicy(LinkedList<string> policy)
        {
            ...
            {
                case "AND":
                    sellingPolicy = new PolicyCompositeAnd(tmp1, tmp2, 0, name);
                    tmp1.policyId = tmp2.policyId + 1;
                    sellingPolicy.policyId = tmp1.policyId + 1;
                    break;
                case "OR":
                    sellingPolicy = new PolicyCompositeOr(tmp1, tmp2, 0, name);
                    tmp1.policyId = tmp2.policyId + 1;
                    sellingPolicy.policyId = tmp1.policyId + 1;
                    break;
                case "XOR":
                    sellingPolicy = new PolicyCompositeXor(tmp1, tmp2, 0, name);
                    tmp1.policyId = tmp2.policyId + 1;
                    sellingPolicy.policyId = tmp1.policyId + 1;
                    break;
                default:
        ...
        private PolicyComponent CreatePolicy(LinkedList<string> policy)
        {
            ...
            {
                case "Ban":
                    createdPolicy = new PolicyLeafBannedUser(policy.ElementAt(3), 0, name);
                    break;
                case "Max":
                    createdPolicy = new PolicyLeafMaximumAmount(Int32.Parse(policy.ElementAt(3)), Int32.Parse(policy.ElementAt(4)), 0, name);
                    break;
                case "Min":
                    createdPolicy = new PolicyLeafMinimumAmount(Int32.Parse(policy.ElementAt(3)), Int32.Parse(policy.ElementAt(4)), 0, name);
                    break;
                default:
